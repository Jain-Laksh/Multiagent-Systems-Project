"""
Training script for Actor-Critic on CartPole-v1
...
def train():
    """
    Main training function for Actor-Critic agent
    """
    ...
    for episode in tqdm(range(config.NUM_EPISODES), desc="Training Progress"):
        ...
        if (episode + 1) % config.SAVE_INTERVAL == 0:
            model_path = os.path.join(config.MODEL_SAVE_PATH, f"actor_critic_model_episode_{episode + 1}.pth")
    ...
    # Save final model
    final_model_path = os.path.join(config.MODEL_SAVE_PATH, "actor_critic_model_final.pth")
    agent.save_model(final_model_path)
    
    # Generate and save plots
    print("\nGenerating training plots...")
    plot_rewards(episode_rewards, config.PLOT_SAVE_PATH, "actor_critic_reward_plot.png")
    plot_rolling_average(episode_rewards, window=100, save_path=config.PLOT_SAVE_PATH, 
                        filename="actor_critic_rolling_average_reward_plot.png")
    plot_training_summary(episode_rewards, config.PLOT_SAVE_PATH, "actor_critic_training_summary.png")
